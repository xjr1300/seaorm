...
Newtypeを定義してそれをモデルフィールドとして使用できます。
次のトレイトが実装されていなくてはなりません。

1. `From<Newtype> for [sea_query::Value](https://docs.rs/sea-query/0.28/sea_query/value/enum.Value.html)`の実装
2. `sea_orm::TryGetablle for Newtype`の実装
3. `sea_query::ValueType for Newtype`の実装
4. `sea_query::Nullable for Newtype`の実装
